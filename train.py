...
def train_cal(config, epoch, model, classifier, clothes_classifier, criterion_cla, criterion_pair,
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids) in enumerate(trainloader):
        ...
        if epoch >= config.TRAIN.START_EPOCH_ADV:
            loss = cla_loss + 0.01 * cla_loss1 + adv_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss
        else:
            loss = cla_loss + 0.01 * cla_loss1 + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss
        optimizer.zero_grad()
        if config.TRAIN.AMP:
    ...
    logger.info('Epoch{0} '
        ...
        cla_loss=batch_cla_loss, cla_loss1=batch_cla_loss1, pair_loss=batch_pair_loss,
