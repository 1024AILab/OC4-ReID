...
def train_cal(config, epoch, model, classifier, clothes_classifier, criterion_cla, criterion_pair,
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids) in enumerate(trainloader):
        ...
        cla_loss1 = criterion_cla(f3, pids)
        ...
        if epoch >= config.TRAIN.START_EPOCH_ADV:
            loss = cla_loss + 0.01 * pair_loss1 + 0.01 * cla_loss1 + adv_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss
        else:
            loss = cla_loss + 0.01 * pair_loss1 + 0.01 * cla_loss1 + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss
        optimizer.zero_grad()
        if config.TRAIN.AMP:
